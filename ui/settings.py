...
class SettingsFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.header = ctk.CTkLabel(self, text="⛭ Settings", font=ctk.CTkFont(size=28, weight="bold"), text_color="#A855F7")
        ...
        # General Settings
        ctk.CTkLabel(self.scroll, text="General", font=ctk.CTkFont(size=20, weight="bold")).pack(anchor="w", padx=20, pady=(20, 10))
        
        self.download_dir_frame = ctk.CTkFrame(self.scroll, fg_color="transparent")
        self.download_dir_frame.pack(fill="x", padx=20, pady=5)
        
        ctk.CTkLabel(self.download_dir_frame, text="Download Folder:", width=150, anchor="w").pack(side="left")
        self.dir_entry = ctk.CTkEntry(self.download_dir_frame, width=300)
        self.dir_entry.pack(side="left", padx=10)
        self.dir_entry.insert(0, config.get("download_dir", ""))
        
        self.browse_btn = ctk.CTkButton(self.download_dir_frame, text="Browse", width=80, command=self.do_browse)
        self.browse_btn.pack(side="left")
        
        # Network Settings
        ctk.CTkLabel(self.scroll, text="Network & Queue", font=ctk.CTkFont(size=20, weight="bold")).pack(anchor="w", padx=20, pady=(20, 10))
        
        self.concurrent_frame = ctk.CTkFrame(self.scroll, fg_color="transparent")
        self.concurrent_frame.pack(fill="x", padx=20, pady=5)
        
        ctk.CTkLabel(self.concurrent_frame, text="Concurrent Downloads:", width=150, anchor="w").pack(side="left")
        self.concurrent_menu = ctk.CTkOptionMenu(self.concurrent_frame, values=["1", "2", "3", "4", "5", "8", "10"])
        self.concurrent_menu.pack(side="left", padx=10)
        self.concurrent_menu.set(str(config.get("concurrent_downloads", 3)))
        
        # Advanced Settings
        ctk.CTkLabel(self.scroll, text="Advanced API Config", font=ctk.CTkFont(size=20, weight="bold")).pack(anchor="w", padx=20, pady=(20, 10))
        
        self.cookies_frame = ctk.CTkFrame(self.scroll, fg_color="transparent")
        self.cookies_frame.pack(fill="x", padx=20, pady=5)
        ctk.CTkLabel(self.cookies_frame, text="Import Cookies From:", width=150, anchor="w").pack(side="left")
        self.cookies_menu = ctk.CTkOptionMenu(self.cookies_frame, values=["None", "Chrome", "Edge", "Firefox", "Opera", "Brave", "Safari"])
        self.cookies_menu.pack(side="left", padx=10)
        self.cookies_menu.set(config.get("cookies_browser", "None"))
        
        # Save mechanism
        self.status_lbl = ctk.CTkLabel(self.scroll, text="", text_color="#10B981")
        self.status_lbl.pack(anchor="e", padx=20, pady=(20, 0))
        
        self.save_btn = ctk.CTkButton(self.scroll, text="Save Settings", height=40, corner_radius=8, fg_color="#10B981", hover_color="#059669", command=self.save_settings)
